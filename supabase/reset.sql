-- Day 1 only. Destroys all data. Re-run npm run seed after.
--
-- supabase/reset.sql — drop everything and rebuild from supabase/schema.sql v1.1.
-- Owner: TM3. Paste into the Supabase SQL editor and run.
--
-- Use this when the database holds nothing but the v1.0 seed, which on Day 1 is
-- the normal case. It is the only path that guarantees the CHECK constraints
-- actually exist: ALTER TABLE ... ADD CONSTRAINT refuses to apply to rows that
-- already violate it, and the v1.0 tables had no constraints at all.
--
-- If the database holds data worth keeping, stop and use
-- supabase/migrations/001_v1_1.sql instead.

-- ── drop, children before parents ────────────────────────────────────────
-- 'cascade' makes the order redundant, but FK-safe order is what you want to
-- read at 2am when one of these fails.
...
-- ═════════════════════════════════════════════════════════════════════════
-- Everything below is supabase/schema.sql verbatim. If you edit one, edit both.
-- ═════════════════════════════════════════════════════════════════════════
